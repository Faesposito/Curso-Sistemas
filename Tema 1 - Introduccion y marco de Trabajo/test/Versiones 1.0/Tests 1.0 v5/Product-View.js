...
let cartView = new ShoppingCartView('sidebar',cartModel);
let cartController = new ShoppingCartController(cartModel, cartView);
	...
	updateFrom( productPromise ) {
		...
		productPromise.then( productArray => {
			...
			document.getElementById(this.id).addEventListener('click', event =>
			{
				...
				if (event.target.classList.contains("addToCart")) cartController.onAddToCartButtonClick(event);
			});
