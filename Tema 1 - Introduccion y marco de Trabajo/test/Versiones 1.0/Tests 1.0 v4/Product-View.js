...
class ProductView
{
	...
	createShoppingCart()
	{
		let cartModel = new ShoppingCartModel();
		let cartView = new ShoppingCartView('ShoppingCartContainner', cartModel);
		
		//cartView.addEventListener('ShoppingCartContainner', event => this.innerController.onCreate() );
	}
	update()
	{
	...
	updateFrom( productPromise )
	{
		productPromise.then( productArray =>
		{
			let innerHTML = 
			`<h1> Products </h1>
			<div id= "ShoppingCartContainner" class="ShoppingCartContainner"></div>
							...
							<div class="product-bottom-details">
								...
								<div class="product-quantity">
									...
									<div class="product-cart">
										<button id="${this.id}AddToCartButton${productArray.indexOf(product)}" class="addToCart" type="submit">Add to Cart</button>
									</div>
								</div> 
							</div>
						</div>    
			...
			innerHTML += 
			`
			</div id="${this.id}ButtonContainner" class="ButtonContainner">
			...
			document.getElementsByClassName('addToCart').addEventListener('click', event => this.innerController.onAddToCartButtonClick(event) );
