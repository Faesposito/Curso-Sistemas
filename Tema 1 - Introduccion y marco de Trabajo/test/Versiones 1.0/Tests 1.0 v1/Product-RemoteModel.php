<?php
	...
	$connection = new PDO('mysql:host=127.0.0.1:3306;dbname=abm', 'root', 'Faespo1493' );
	...
	function create( $connection, $data )
	{
		if ( isValidProductData( $data ) && !isProductAlredyCreated($connection, $data))
		{
			try
			{
				$SQLCode = "INSERT INTO product(name,type,price,quantity) VALUES('$data->name','$data->type','$data->price','$data->quantity' )";
	...
	function edit( $connection, $data )
	{
		$SQLCode = "UPDATE product
					SET name='$data->name', type='$data->type',	price='$data->price', quantity='$data->quantity'
	...
	function isValidProductData( $userData )
	{
		...
		$success = ( $success && array_key_exists('type', $userData ) && !is_null( $userData->type ) );
